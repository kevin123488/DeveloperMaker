...
public class AiInterviewController {
    ...
    public ResponseEntity<BaseResponseBody> analyzeInterview(@PathVariable Long aiqId, @RequestPart(value = "file") @ApiParam(value = "이미지 파일") MultipartFile file, @RequestPart(value = "aiInterviewRequestDto") AiInterviewRequestDto aiInterviewRequestDto) throws ParseException {
        String imgUrl = awsS3Service.uploadImage(file);
        return ResponseEntity.status(200).body(BaseResponseBody.of(200, "Success", aiInterviewService.getResult(aiqId, imgUrl, aiInterviewRequestDto)));
    }

    @PostMapping("/isFaceImage")
    @ApiOperation(value = "이미지 얼굴 인식 여부 반환", notes = "jpg를 보내면 얼굴 인식 여부를 반환.")
    public ResponseEntity<BaseResponseBody> testImage(@RequestPart(value = "file") @ApiParam(value = "이미지 파일") MultipartFile file) throws ParseException {
        String imgUrl = awsS3Service.uploadImage(file);
